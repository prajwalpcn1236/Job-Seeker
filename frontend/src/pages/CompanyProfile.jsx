...
const CompanyProfile = () => {
  ...
  const fetchCompany = async () => {
    ...
    // Add a check to ensure `id` is a valid ObjectId format (24-character hex string)
    if (id && id.match(/^[0-9a-fA-F]{24}$/)) {
      console.log("Using Params ID:", id); // Debugging log
    } else {
      id = user?._id; // Fallback to user._id if params.id is invalid
      console.log("Using User ID as fallback:", id); // Debugging log
    }
    ...
    if (id && id.match(/^[0-9a-fA-F]{24}$/)) {
      try {
        ...
        console.error("API request error:", error); // Use console.error for errors
      } finally {
        setIsLoading(false); // Ensure setIsLoading(false) is called in finally block
      }
    } else {

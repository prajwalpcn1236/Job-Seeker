...
const onSubmit = async (data) => {
  setIsLoading(true);
  setErrMsg(null);
  const newData = { ...data, jobType: jobType,desc:data.desc };
  try {
...
const getRecentPost = async () => {
  try {
    ...
    console.log("Recent Posts:", res?.data?.jobPosts);
  } catch (error) {
    console.log(error);
  }
...
return (
    <div className='container pt-16  mx-auto flex flex-col md:flex-row gap-8 2xl:gap-14 bg-[#f7fdfd] px-5 '>
    ...
    <div className='w-full md:w-1/3 2xl:2/4 p-5 mt-20 md:mt-0'>
      ...
      <div className='w-full flex flex-wrap gap-6'>
        {recentPost.slice(0, 4).map((job, index) => {
          ...
          return (<JobCard
            job={data}
            key={index} // Assuming each job has a unique id for a better key than index
          />);
         } )}
      </div>
    </div>
